...
namespace Kortros.Utilities
{
    public class Logger
    {
        private static readonly Lazy<ILog> log = new Lazy<ILog>(() =>
        {
            ...
            string logName = dateTime + " " + userName + " " + "Updater.log";
            ...
            var configStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Kortros.log4net.config");
            ...
            var fileAppender = appenders.OfType<log4net.Appender.FileAppender>().FirstOrDefault(appender => appender.Name == "Updater");
            ...
            {
                throw new InvalidOperationException("Appender с именем 'Updater' не найден в конфигурационном файле.");
            }
            return logger ;
        });
        ...
        public static void CreateLogFolderAndCopy()
        {
            ...
            if (File.Exists(sourceFilePath))
            {
                ...
                Logger.Log.Info("Файл лога Updater скопирован на внешний диск");
            }
            else
